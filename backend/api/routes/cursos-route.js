const Route = require('@api/routes/route'),
      ufrn = require('@crawler/ufrn'),
      CursosModel = require('@models/cursos-model');

class CursosRoute extends Route {
  /**
    Classe da rota para dados das Cursos

    @description
    Essa Class é responsável por gerenciar as requisições para a rota /cursos

    @param {Express} expressApp Aplicação instancia de expressApp
    @param {String} url Url para acesso via HTTP
  */
  constructor(expressApp) {
    super(expressApp, '/cursos')
  }

  /**
    onGet

    @description
    Função para processamento da rota durante requisções GET

    @returns {Promise}
  */
  async onGet(request) {
    let data = await ufrn.getAndProcess(ufrn.urls.cursos,CursosModel)

    return {
      length: data.length,
      hits: data
    };
  }
}

module.exports = CursosRoute;
